...
async function expandFiles(searchPath, filePattern) {
  ...
  if (stat.isFile()) {
    ...
    if (glob.hasMagic(filePattern)) {
      return glob.sync(filePattern, { cwd: path.dirname(searchPath), matchBase: true, absolute: true })
    ...
    return await new Promise((resolveGlob, reject) => {
      glob(
        path.posix.join(path.relative(process.cwd(), searchPath).replace(/\\/g, '/'), '**', filePattern),
