...
async function expandFiles(searchPath, filePattern) {
  ...
  if (stat.isFile()) {
    ...
    // Fix: instead of trying to use glob sync with "default" export, use .globSync for glob@10+
    const matchBase = true;
    const matches = glob.hasMagic(filePattern) || filePattern !== '*.*'
      ? glob.globSync(filePattern, { cwd: path.dirname(searchPath), matchBase, absolute: true })
      : [searchPath];
    if (matches.map(f => path.resolve(f)).includes(path.resolve(searchPath))) {
      return [searchPath];
    } else if (!glob.hasMagic(filePattern) && path.basename(searchPath) === filePattern) {
      return [searchPath];
    } else {
      return [];
    }
    ...
    // For glob@10+, prefer callback API ("glob" is a function)
